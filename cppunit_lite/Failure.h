

#ifndef FAILURE_H
#define FAILURE_H

// Failure records the circumstances surrounding a test failure.  Using C++
// macros were are able to record the name of the file where the failure 
// occurred, the line number, and the text of the condition which provoked
// the failure.

 
#include <string>
#include <iostream>


class Failure
{
public:
	Failure (std::string theCondition, std::string theTestName, std::string theFileName, long theLineNumber, std::string _msg) 
		: condition (theCondition), testName (theTestName), fileName (theFileName), lineNumber (theLineNumber), msg(_msg)
	{
	}

	Failure (std::string theCondition, std::string theTestName, std::string theFileName, long theLineNumber ) 
		: condition (theCondition), testName (theTestName), fileName (theFileName), lineNumber (theLineNumber) 
	{
	}

	std::string condition;
	std::string testName;
	std::string fileName;
	std::string msg;
	long lineNumber;
};

 
inline std::ostream& operator<< (std::ostream& stream, const Failure& failure)
{
	stream 
		<< "Fail:\t"<<failure.fileName.c_str ()
		<< " (" << failure.lineNumber << "): " << failure.msg.c_str ()
		<< "\t Failure_Msg: \"" << failure.condition.c_str () << "\" "; 
		//<< std::endl;

	return stream;
}  

#endif

